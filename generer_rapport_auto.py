...
FORM_ID      = "1vHAjeq5a5igtV4SwSIO-eZr8dJF3LAo64w0eYGLXsCs"
...
SECTION_MAP = [
    (["organisation", "processus", "organisez-vous"],  "organisation"),
    (["outils", "automatisation", "façon de travailler"], "outils"),
    (["ventes", "soumissions"],                        "ventes"),
    (["finance", "rentabilit"],                        "finances"),
    (["quipe", "ressources humaines", "recrutement"],  "equipe"),
    (["vision", "ambitions", "objectifs"],             "vision"),
]
...
def _build_form_index(service):
    ...
    for item in items:
        ...
        if current_key is None:
            t = title_raw.lower()
            if "nom du dirigeant" in t or "nom du client" in t or "prénom et nom" in t or "prenom et nom" in t:
                id_champs["nom"] = qid
            elif "entreprise" in t:
...
def _patch_and_generate(nom, entreprise, secteur, scores, textes, date_rapport=None, reponses_txt=None):
    """
    ...
    # ── Dossier client + fichiers dans RAPPORTS_CLIENTS/ ──────────────────
    import shutil as _shutil, re as _re, datetime as _dt, csv as _csv
    _date_str    = (date_rapport or _dt.date.today().isoformat())[:10]
    _score_global = round(sum(scores.values()) / (20 * len(scores)) * 100)

    _nom_safe = _re.sub(r'[^a-zA-ZÀ-ÿ0-9]', '_', nom).strip('_')
    _ent_safe = _re.sub(r'[^a-zA-ZÀ-ÿ0-9]', '_', entreprise).strip('_')
    _client_dir = os.path.expanduser(
        f"~/Documents/prospect2/RAPPORTS_CLIENTS/{_nom_safe}_{_ent_safe}"
    )
    os.makedirs(_client_dir, exist_ok=True)

    # rapport_diagnostic.pdf
    _src = os.path.expanduser("~/Documents/prospect2/rapport_diagnostic_TEST.pdf")
    _shutil.copy2(_src, os.path.join(_client_dir, "rapport_diagnostic.pdf"))

    # reponses_formulaire.txt
    if reponses_txt:
        with open(os.path.join(_client_dir, "reponses_formulaire.txt"), "w", encoding="utf-8") as _f:
            _f.write(reponses_txt)

    # statut.txt
    _statut_path = os.path.join(_client_dir, "statut.txt")
    if not os.path.exists(_statut_path):
        with open(_statut_path, "w", encoding="utf-8") as _f:
            _f.write(f"Statut      : À envoyer\n"
                     f"Généré le   : {_date_str}\n"
                     f"Envoyé le   :\n"
                     f"Méthode     :\n"
                     f"Notes       :\n")
    ...
    print(f"  Dossier client  : RAPPORTS_CLIENTS/{_folder_name}/")
    print(f"    rapport_diagnostic.pdf")
    print(f"    reponses_formulaire.txt")
    print(f"    statut.txt")
    print(f"  suivi_global.csv mis à jour")
    print("=" * 62)
...
def main():
    ...
    # Export réponses brutes pour le dossier client
    import re as _re2, datetime as _dt2
    _date_gen = response.get("createTime", _dt2.date.today().isoformat())[:10]
    _lines = [
        f"RÉPONSES AU FORMULAIRE DIAGNOSTIC — PROSPECT 2.0",
        "=" * 60,
        f"Client     : {nom}",
    ...
    for sk, qs in sections.items():
        _lines += ["", f"── {SECTION_KEY_TO_LABEL.get(sk, sk)} ──", ""]
        for q in qs:
            ans_block = response.get("answers", {}).get(q["qid"], {})
            raw_vals  = [a["value"] for a in ans_block.get("textAnswers", {}).get("answers", [])]
            if not raw_vals:
                continue
            _lines.append(f"Q : {q['title']}")
            _lines.append(f"R : {' / '.join(raw_vals)}")
            _lines.append("")
    _reponses_txt = "\n".join(_lines)

    # Génération PDF avec actions Claude
    _patch_and_generate(nom, entreprise, secteur, scores, textes,
                        reponses_txt=_reponses_txt)
